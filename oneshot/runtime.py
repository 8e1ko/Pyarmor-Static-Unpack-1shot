...
class RuntimeInfo:
    def __init__(self, file_path: str) -> None:
        ...
        if file_path.endswith(".pyd"):
            self._extract_info_win64()
        else:
            # TODO: implement for other platforms
            self._extract_info_win64()

        self.serial_number = self.part_1[12:18].decode("utf-8", errors="replace")
        self.runtime_aes_key = self._calc_aes_key()
        self.mix_str_aes_nonce = self._mix_str_aes_nonce()
    ...
    def _extract_info_win64(self) -> None:
        """
    ...
    def _calc_aes_key(self) -> bytes:
        return hashlib.md5(
            self.part_1 + self.part_2 + self.part_3 + GLOBAL_CERT
        ).digest()

    def _mix_str_aes_nonce(self) -> bytes:
    ...
    def default(cls) -> "RuntimeInfo":
        ...
        instance.runtime_aes_key = instance._calc_aes_key()
        instance.mix_str_aes_nonce = instance._mix_str_aes_nonce()
        return instance
